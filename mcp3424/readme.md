# MCP3424

Portable, non-blocking driver for the Microchip MCP3424
18-bit, 4-channel I²C analog-to-digital converter.

The library is part of the STM32 Universal Libraries framework and
provides a unified API compatible with other ADC libraries.

---

# Features

- Pure C (C11)
- Multi-instance
- Hardware independent
- Non-blocking state machine
- Blocking and interrupt I²C support
- Single channel measurements
- Sequential channel scanning
- One-shot mode
- Continuous mode
- 12, 14, 16 and 18-bit resolution
- Programmable Gain Amplifier (PGA ×1, ×2, ×4, ×8)
- Voltage calculation in millivolts
- Communication diagnostics

---

# Requirements

- communication_i2c_driver_t
- STM32 HAL driver examples included

---

# Example

See:

```
examples/STM32CubeIDE/
```

---

# API

The complete API documentation is available in:

```
Inc/mcp3424.h
```

---

# License

MIT License